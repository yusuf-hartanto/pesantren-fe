...
import { deleteKelasMda, fetchKelasMdaPage, postKelasMdaUpdate, postExport, resetRedux } from '../slice/index'
    ...
    <TableCell size='small'>
      ...
      >
        <MenuItem
          component={Link}
          sx={{ '& svg': { mr: 2 } }}
          href={`/app/kelas-mda/form?id=${data.row.id_kelas_mda}&view=true`}
          onClick={handleView}
        >
        ...
        {canEdit && [
          <MenuItem
            ...
            component={Link}
            sx={{ '& svg': { mr: 2 } }}
            href={`/app/kelas-mda/form?id=${data.row.id_kelas_mda}`}
            onClick={handleView}
          >
        ...
        <DialogDelete
          ...
          handleOk={() => {
            handleDelete(data.row.id_kelas_mda)
            setOpenConfirm(false)
          }}
          handleClose={() => {
            setOpenConfirm(false)
...
const Table = () => {
  ...
  useEffect(() => {
    if (store.delete) {
      dispatch(fetchKelasMdaPage({ page: 1, perPage: perPage, q: filter, status: selectedStatus?.value }))
      dispatch(resetRedux())
    }
  ...
  const handleChangePage = useCallback(
    (newPage: number) => {
      setPage(newPage)
      dispatch(fetchKelasMdaPage({ page: newPage, perPage: perPage, q: filter, status: selectedStatus?.value }))
    },
  ...
  const onAddForm = () => {
    router.replace('/app/kelas-mda/form')
  }

  const onImport = () => {
    router.replace('/app/kelas-mda/import')
  }
    ...
    dispatch(
      postKelasMdaUpdate({
        ...
        params: {
          ...
          id_tingkat: {
            value: data.id_tingkat
          },
          id_lembaga: {
            value: data.id_lembaga
          }
        }
      })
    )
  ...
  const handleChangePerPage = (event: any) => {
    ...
    dispatch(fetchKelasMdaPage({ page: 1, perPage: newPerPage, q: filter, status: selectedStatus?.value }))
  }
  ...
  return (
    <Grid container spacing={6} sx={{ width: '100%' }}>
      ...
      <Grid size={12}>
        <Card>
          <CardHeader title='Kelas MDA' sx={{ paddingBottom: 0 }} />
          <Toolbar
            sx={{
              px: '1.5rem !important',
              minHeight: 'auto',
