...
import { postBatchStatusAwalSantri, postExport, postImport, resetRedux } from '../slice'
...
export default function ImportExcelPage() {
  ...
  const handleCommit = async () => {
    ...
    try {
      ...
      const res = await dispatch(postBatchStatusAwalSantri({ data: payloads })).unwrap()
          ...
          <table className='min-w-full text-sm'>
            <thead className='bg-gray-100 text-left'>
              <tr>
                ...
                <th className='px-3 py-2'>Kode Status Awal</th>
                <th className='px-3 py-2'>Nama Status Awal</th>
            ...
            <tbody>
              {result.data.map(row => (
                <tr key={row.row} className={row.valid ? 'border-t bg-white' : 'border-t bg-red-50'}>
                  ...
                  <td className='px-3 py-2 font-medium'>{row.payload.kode_status_awal}</td>
                  <td className='px-3 py-2 font-medium'>{row.payload.nama_status_awal}</td>
        ...
        <div className='bg-white rounded-xl shadow-sm border p-6 space-y-6'>
          <div className='border border-blue-200 bg-blue-50 rounded-lg p-4 text-sm text-blue-700'>
            ...
            <ul className='list-disc list-inside space-y-1'>
              ...
              <li>
                Jika <b>Kode Status Awal</b> kosong → INSERT (data baru)
              </li>
              <li>
                Jika <b>Kode Status Awal</b> ada → UPDATE (perbarui data)
              </li>
