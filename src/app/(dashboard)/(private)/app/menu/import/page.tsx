...
import { postBatchMenu, postExport, postImport, resetRedux } from '../slice'
...
export default function ImportExcelPage() {
  ...
  const handleConfirmCommit = async () => {
    ...
    try {
      ...
      const res = await dispatch(postBatchMenu({ data: payloads })).unwrap()
              ...
              {result.data.map((row) => (
                ...
                >
                  ...
                  <div className="space-y-1">
                    <div>
                      <span className="text-gray-500">Nama Menu</span>
                      <div className="font-medium">
                        {row.payload.menu_name}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div>{row.payload.menu_icon ?? '-'}</div>
                    </div>

                    <div>
                      <span className="text-gray-500">URL</span>
                      <div>{row.payload.module_name ?? '-'}</div>
                    </div>

                    <div className="flex gap-4">
                      <div>
                        <span className="text-gray-500">Nomor Urut</span>
                        <div>{row.payload.seq_number ?? '-'}</div>
                      </div>
                      <div>
              ...
              <table className="min-w-full text-sm">
                <thead className="bg-gray-100 text-left">
                  <tr>
                    ...
                    <th className="px-3 py-2">Nama Menu</th>
                ...
                <tbody>
                  {result.data.map((row) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-2">
                        {row.payload.menu_icon ?? '-'}
                      </td>
                      <td className="px-3 py-2">
                        {row.payload.module_name ?? '-'}
                      </td>
                      <td className="px-3 py-2">
                        {row.payload.seq_number ?? '-'}
                      </td>
                      <td className="px-3 py-2">
                        {row.payload.status == 1 ? 'Aktif' : 'Nonaktif'}
                      </td>
        ...
        <div className="bg-white rounded-xl shadow-sm border p-6 space-y-6">
          <div className="border border-blue-200 bg-blue-50 rounded-lg p-4 text-sm text-blue-700">
            ...
            <ul className="list-disc list-inside space-y-1">
              ...
              <li>Jika <b>menu_name</b> kosong → INSERT (data baru)</li>
              <li>Jika <b>menu_name</b> ada → UPDATE (perbarui data)</li>
