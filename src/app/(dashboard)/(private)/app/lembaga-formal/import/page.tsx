...
/** Import slice sesuai modul lembaga formal */
import { postBatch, postExport, postImport, resetRedux } from '../slice'

/** 1. Penyesuaian Interface berdasarkan Response Lembaga Formal */
...
export default function ImportExcelLembagaFormalPage() {
  ...
  /** Navigasi kembali ke list lembaga formal */
  ...
  const handleCommit = async () => {
    ...
    try {
      ...
      toast.success('Import data lembaga formal berhasil')
      onCancel()
    } catch (e) {
      console.error(e)
  ...
  /** 2. Penyesuaian Tabel Preview Kolom Lembaga Formal */
          ...
          <table className='min-w-full text-[11px]'>
            <thead className='bg-gray-100 text-left'>
              <tr>
                ...
                <th className='px-3 py-2'>Jenis / Akreditasi</th>
              ...
              {result.data.map(row => (
                <tr key={row.row} className={row.valid ? 'border-t bg-white' : 'border-t bg-red-50'}>
                  ...
                  <td className='px-3 py-2'>
                    {row.payload.jenis_lembaga} <br />
                    <span className='text-gray-400 text-[10px]'>Akred: {row.payload.status_akreditasi}</span>
                  </td>
                  <td className='px-3 py-2'>{row.payload.nomor_npsn}</td>
                  <td className='px-3 py-2'>
                    <span className='text-[10px] text-gray-500 font-mono'>{row.payload.id_cabang}</span>
                  </td>
                  <td className='px-3 py-2 max-w-[150px] truncate'>{row.payload.keterangan || '-'}</td>
                  <td className='px-3 py-2'>
                    {row.valid ? (
                      <span className='text-green-600 font-semibold'>✓</span>
                    ) : (
                      <span className='text-red-600 leading-tight block'>{row.error}</span>
                    )}
                  </td>
                </tr>
              ))}
  ...
  return (
    <div className='min-h-screen bg-gray-50 py-10 px-4'>
      <div className='max-w-4xl mx-auto'>
        <div className='mb-6'>
          <Link
            href='/app/lembaga-formal/list'
            className='inline-flex items-center gap-2 text-sm text-gray-600 hover:text-gray-900'
          >
            <i className='tabler-arrow-back-up'></i> Kembali ke List Lembaga
          </Link>
          <h1 className='text-xl font-semibold mt-2'>Import Lembaga Pendidikan Formal</h1>
        </div>
        ...
        <div className='bg-white rounded-xl shadow-sm border p-6 space-y-6'>
          ...
          <div className='border border-blue-200 bg-blue-50 rounded-lg p-4 text-sm text-blue-700'>
            <h3 className='font-medium mb-2'>Petunjuk Import Lembaga Formal</h3>
            <ul className='list-disc list-inside space-y-1'>
              <li>Gunakan format file Excel terbaru (.xlsx)</li>
              <li>
                Pengecekan duplikasi dilakukan berdasarkan <b>Nama Lembaga</b>.
              </li>
              <li>Field wajib: Nama Lembaga, Jenis Lembaga, Akreditasi, NPSN, dan ID Cabang.</li>
              <li>Jenis Lembaga harus sesuai (SD, MI, SMP, MTs, SMA, MA, SMK, Diniyah, atau Perguruan Tinggi).</li>
              <li>
                Mode <b>Commit</b> akan memproses penyimpanan permanen ke database.
              </li>
            </ul>
            <div className='mt-3'>
              <a className='text-blue-600 underline hover:text-blue-400 cursor-pointer' onClick={downloadTemplate}>
                <b>Download Template Excel Lembaga Formal</b>
              </a>
            </div>
          </div>
