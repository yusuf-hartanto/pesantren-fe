...
import { deleteOrangTuaWali, fetchOrangTuaWaliPage, postExport, resetRedux } from '../slice/index'
...
function RowAction(data: any) {
  ...
  const handleDelete = (id: string) => {
    dispatch(deleteOrangTuaWali(id))
    optionsOnClose()
  }
...
const Table = () => {
  ...
  useEffect(() => {
    if (store.delete) {
      dispatch(fetchOrangTuaWaliPage({ page: 1, perPage: perPage, q: filter }))
  ...
  const onAddForm = () => {
    router.replace('/app/orang-tua-wali/form')
  }

  const onImport = () => {
    router.replace('/app/orang-tua-wali/import')
  }
  ...
  const handleChangePage = (newPage: number) => {
    setPage(newPage)
    dispatch(fetchOrangTuaWaliPage({ page: newPage, perPage: perPage, q: filter }))
  }
  ...
  const handleChangePerPage = (event: any) => {
    ...
    dispatch(fetchOrangTuaWaliPage({ page: 1, perPage: newPerPage, q: filter }))
  }
    ...
    if (dataPage) {
      ...
      return {
        ...
        fields: [
          ...
          tableColumn('PENDIDIKAN', 'pendidikan'),
          tableColumn('PEKERJAAN', 'pekerjaan'),
          tableColumn('ALAMAT', 'alamat'),
  ...
  return (
    <Grid container spacing={6} sx={{ width: '100%' }}>
      <Grid size={12}>
        <Card>
          <CardHeader title='Orang Tua Wali' sx={{ paddingBottom: 0 }} />
          <Toolbar
            sx={{
              px: '1.5rem !important',
              minHeight: 'auto',
