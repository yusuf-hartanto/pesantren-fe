...
import { postBatchOrangTuaWali, postExport, postImport, resetRedux } from '../slice'
...
export default function ImportExcelPage() {
  ...
  const handleCommit = async () => {
    ...
    try {
      ...
      const res = await dispatch(postBatchOrangTuaWali({ data: payloads })).unwrap()
          ...
          <table className='min-w-full text-sm'>
            <thead className='bg-gray-100 text-left'>
              <tr>
                ...
                <th className='px-3 py-2'>Kota/Kabupatan</th>
            ...
            <tbody>
              {result.data.map(row => (
                <tr key={row.row} className={row.valid ? 'border-t bg-white' : 'border-t bg-red-50'}>
                  ...
                  <td className='px-3 py-2 font-medium'>{row.payload.nama_wali}</td>
                  <td className='px-3 py-2 font-medium'>{row.payload.hubungan}</td>
                  <td className='px-3 py-2 font-medium'>{row.payload.nik}</td>
                  <td className='px-3 py-2 font-medium'>{row.payload.pendidikan}</td>
                  <td className='px-3 py-2 font-medium'>{row.payload.pekerjaan}</td>
                  <td className='px-3 py-2'>{row.payload.penghasilan}</td>
                  <td className='px-3 py-2'>{row.payload.no_hp}</td>
                  <td className='px-3 py-2'>{row.payload.alamat}</td>
                  <td className='px-3 py-2'>{row.payload.provinsi}</td>
                  <td className='px-3 py-2'>{row.payload.kabupaten}</td>
                  <td className='px-3 py-2'>{row.payload.kecamatan}</td>
                  <td className='px-3 py-2'>{row.payload.kelurahan}</td>
        ...
        <div className='bg-white rounded-xl shadow-sm border p-6 space-y-6'>
          <div className='border border-blue-200 bg-blue-50 rounded-lg p-4 text-sm text-blue-700'>
            ...
            <ul className='list-disc list-inside space-y-1'>
              ...
              <li>
                Jika <b>Nama Wali</b> kosong → INSERT (data baru)
              </li>
              <li>
                Jika <b>Nama Wali</b> ada → UPDATE (perbarui data)
              </li>
