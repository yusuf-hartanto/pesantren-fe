...
import { deleteTahunAjaran, fetchTahunAjaranPage, postTahunAjaranUpdate, postExport, resetRedux } from '../slice/index'
...
const Table = () => {
  ...
  useEffect(() => {
    if (store.delete) {
      dispatch(fetchTahunAjaranPage({ page: 1, perPage: perPage, q: filter, status: selectedStatus?.value }))
      dispatch(resetRedux())
    }
  ...
  const handleChangePage = useCallback(
    (newPage: number) => {
      setPage(newPage)
      dispatch(fetchTahunAjaranPage({ page: newPage, perPage: perPage, q: filter, status: selectedStatus?.value }))
    },
  ...
  const onImport = () => {
    router.replace('/app/tahun-ajaran/import')
  }
  ...
  const handleChangePerPage = (event: any) => {
    ...
    dispatch(fetchTahunAjaranPage({ page: 1, perPage: newPerPage, q: filter, status: selectedStatus?.value }))
  }
