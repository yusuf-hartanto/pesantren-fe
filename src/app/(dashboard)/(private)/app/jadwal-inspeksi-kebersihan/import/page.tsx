...
import { postBatchJadwalInspeksiKebersihan, postExport, postImport, resetRedux } from '../slice'
...
export default function ImportExcelPage() {
  ...
  const handleCommit = async () => {
    ...
    try {
      ...
      const res = await dispatch(postBatchJadwalInspeksiKebersihan({ data: payloads })).unwrap()
            ...
            <tbody>
              {result.data.map(row => (
                <tr key={row.row} className={row.valid ? 'border-t bg-white' : 'border-t bg-red-50'}>
                  ...
                  <td className='px-3 py-2 font-medium text-nowrap'>{row.payload.cabang}</td>
                  <td className='px-3 py-2 font-medium'>{row.payload.hari_text}</td>
                  <td className='px-3 py-2 font-medium'>{row.payload.kode_slot}</td>
                  <td className='px-3 py-2 font-medium text-nowrap'>
                    {row.payload.jam_mulai?.slice(0, -3)} - {row.payload.jam_selesai?.slice(0, -3)}
                  </td>
                  <td className='px-3 py-2 font-medium text-nowrap'>{row.payload.petugas}</td>
  ...
  return (
    <div className='min-h-screen bg-gray-50 py-10 px-4'>
      <div className='max-w-3xl mx-auto'>
        <div className='mb-6'>
          <Link
            href='/app/jadwal-inspeksi-kebersihan/list'
        ...
        <div className='bg-white rounded-xl shadow-sm border p-6 space-y-6'>
          <div className='border border-blue-200 bg-blue-50 rounded-lg p-4 text-sm text-blue-700'>
            ...
            <ul className='list-disc list-inside space-y-1'>
              ...
              <li>
                Jika <b>Cabang</b> , <b>Hari</b> dan <b>Kode Slot</b> kosong → INSERT (data baru)
              </li>
              <li>
                Jika <b>Cabang</b> , <b>Hari</b> dan <b>Kode Slot</b> ada → UPDATE (perbarui data)
              </li>
