...
import { deleteGuruMataPelajaran, fetchGuruMataPelajaranPage, postExport, resetRedux } from '../slice/index'
...
function RowAction(data: any) {
  ...
  const handleDelete = (id: string) => {
    dispatch(deleteGuruMataPelajaran(id))
    optionsOnClose()
  }
    ...
    <TableCell size='small'>
      ...
      >
        <MenuItem
          component={Link}
          sx={{ '& svg': { mr: 2 } }}
          href={`/app/guru-mata-pelajaran/form?id=${data.row.id_jenisguru}&view=true`}
          onClick={handleView}
        >
        ...
        {canEdit &&
          <MenuItem
            ...
            component={Link}
            sx={{ '& svg': { mr: 2 } }}
            href={`/app/guru-mata-pelajaran/form?id=${data.row.id_jenisguru}`}
            onClick={handleView}
          >
        ...
        {canDelete && [
          ...
          <DialogDelete
            ...
            handleOk={() => {
              handleDelete(data.row.id_jenisguru)
              setOpenConfirm(false)
            }}
            handleClose={() => {
              setOpenConfirm(false)
...
const Table = () => {
  ...
  useEffect(() => {
    if (store.delete) {
      dispatch(fetchGuruMataPelajaranPage({ page: 1, perPage: perPage, q: filter }))
  ...
  const onAddForm = () => {
    router.replace('/app/guru-mata-pelajaran/form')
  }

  const onImport = () => {
    router.replace('/app/guru-mata-pelajaran/import')
  }
  ...
  const handleChangePage = (newPage: number) => {
    setPage(newPage)
    dispatch(fetchGuruMataPelajaranPage({ page: newPage, perPage: perPage, q: filter }))
  }
  ...
  const handleChangePerPage = (event: any) => {
    ...
    dispatch(fetchGuruMataPelajaranPage({ page: 1, perPage: newPerPage, q: filter }))
  }
  ...
  return (
    <Grid container spacing={6} sx={{ width: '100%' }}>
      <Grid size={12}>
        <Card>
          <CardHeader title='Guru Mata Pelajaran' sx={{ paddingBottom: 0 }} />
          <Toolbar
            sx={{
              px: '1.5rem !important',
              minHeight: 'auto',
