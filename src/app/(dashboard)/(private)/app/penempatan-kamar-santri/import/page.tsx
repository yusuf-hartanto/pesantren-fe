...
export default function ImportExcelPage() {
  ...
  const handleConfirmCommit = async () => {
    ...
    .map(d => {
      const { santri, nama_lokasi, tahun_ajaran, ...payload } = d.payload;
                ...
                >
                  ...
                  <div className="space-y-1">
                    ...
                    <div>
                      ...
                      <div className="font-medium">
                        {row.payload.nama_lokasi}
                      </div>
                    </div>

                    <div>
                      <span className="text-gray-500">Tahun Ajaran</span>
                      <div>{row.payload.tahun_ajaran ?? '-'}</div>
                    </div>

                    <div>
                      <span className="text-gray-500">Masuk</span>
                      <div>{row.payload.tanggal_masuk ?? '-'}</div>
                    </div>

                    <div>
                      <span className="text-gray-500">Keluar</span>
                      <div>{row.payload.tanggal_keluar ?? '-'}</div>
                    </div>

                    <div>
                      <span className="text-gray-500">Status</span>
                      <div>
                        {row.payload.status == 'Aktif' ? 'Aktif' : 'Non-Aktif'}
                      </div>
                    </div>
                  ...
                  {result.data.map((row) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-2">
                        {row.payload.tahun_ajaran ?? '-'}
                      </td>
                      <td className="px-3 py-2">
                        {row.payload.tanggal_masuk ?? '-'}
                      </td>
                      <td className="px-3 py-2">
                        {row.payload.tanggal_keluar ?? '-'}
                      </td>
                      <td className="px-3 py-2">
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-10 px-4">
      <div className="mx-auto">
        <div className="mb-6">
          <Link
            href="/app/penempatan-kamar-santri/list"
        ...
        <div className="bg-white rounded-xl shadow-sm border p-6 space-y-6">
          <div className="border border-blue-200 bg-blue-50 rounded-lg p-4 text-sm text-blue-700">
            ...
            <ul className="list-disc list-inside space-y-1">
              ...
              <li>Jika <b>santri, lokasi dan tahun ajaran</b> belum ada → INSERT (data baru)</li>
              <li>Jika <b>santri, lokasi dan tahun ajaran</b> sudah ada → UPDATE (perbarui data)</li>
