...
import { postBatchTingkat, postExport, postImport, resetRedux } from '../slice'
...
export default function ImportExcelPage() {
  ...
  const handleCommit = async () => {
    ...
    try {
      ...
      const res = await dispatch(postBatchTingkat({ data: payloads })).unwrap()
            ...
            <tbody>
              {result.data.map(row => (
                <tr key={row.row} className={row.valid ? 'border-t bg-white' : 'border-t bg-red-50'}>
                  ...
                  <td className='px-3 py-2'>{row.payload.tingkat_type}</td>
