...
import { postBatchKelasFormal, postExport, postImport, resetRedux } from '../slice'
...
export default function ImportExcelPage() {
  ...
  const onCancel = useCallback(() => {
    dispatch(resetRedux())
    router.replace('/app/kelas-formal/list')
  }, [dispatch, router])
  ...
  const handleCommit = async () => {
    ...
    try {
      ...
      const res = await dispatch(postBatchKelasFormal({ data: payloads })).unwrap()
            ...
            <tbody>
              {result.data.map(row => (
                <tr key={row.row} className={row.valid ? 'border-t bg-white' : 'border-t bg-red-50'}>
                  ...
                  <td className='px-3 py-2 font-medium'>{row.payload.nama_kelas}</td>
                  <td className='px-3 py-2 font-medium'>{row.payload.lembaga}</td>
                  <td className='px-3 py-2 font-medium'>{row.payload.tahun_ajaran}</td>
                  <td className='px-3 py-2 font-medium'>{row.payload.tingkat}</td>
                  <td className='px-3 py-2 font-medium'>{row.payload.wali_kelas}</td>
                  <td className='px-3 py-2'>{row.payload.status}</td>
                  <td className='px-3 py-2'>{row.payload.nomor_urut}</td>
        ...
        <div className='bg-white rounded-xl shadow-sm border p-6 space-y-6'>
          <div className='border border-blue-200 bg-blue-50 rounded-lg p-4 text-sm text-blue-700'>
            ...
            <ul className='list-disc list-inside space-y-1'>
              ...
              <li>
                Jika <b>Nama Kelas</b> , <b>Lembaga</b> dan <b>Tahun Ajaran</b> kosong → INSERT (data baru)
              </li>
              <li>
                Jika <b>Nama Kelas</b> , <b>Lembaga</b> dan <b>Tahun Ajaran</b> ada → UPDATE (perbarui data)
              </li>
