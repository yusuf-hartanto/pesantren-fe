...
import { postBatchCabang, postExportCabang, postImportCabang, resetRedux } from '../slice'
...
export default function ImportExcelCabangPage() {
  ...
  /** Navigasi kembali ke list cabang */
  ...
  const downloadTemplate = async () => {
    try {
      const res = await dispatch(postExportCabang({ template: 1 })).unwrap()
  ...
  const onSubmit = async () => {
    ...
    try {
      setLoading(true)
      const res = await dispatch(postImportCabang(formData)).unwrap()
  ...
  const handleCommit = async () => {
    ...
    try {
      ...
      const res = await dispatch(postBatchCabang({ data: payloads })).unwrap()
      ...
      toast.success('Import data cabang berhasil')
      onCancel()
    } catch (e) {
      console.error(e)
  ...
  /** 2. Penyesuaian Tabel Preview Kolom Cabang */
          ...
          <table className='min-w-full text-xs'>
            <thead className='bg-gray-100 text-left'>
              <tr>
                ...
                <th className='px-3 py-2'>Nama Cabang</th>
                <th className='px-3 py-2'>Wilayah (Prov/Kota)</th>
            ...
            <tbody>
              {result.data.map(row => (
                <tr key={row.row} className={row.valid ? 'border-t bg-white' : 'border-t bg-red-50'}>
                  ...
                  <td className='px-3 py-2 font-medium'>{row.payload.nama_cabang}</td>
                  <td className='px-3 py-2'>
                    <div className='text-[10px] text-gray-500'>
                      P: {row.payload.province_id || '-'} / K: {row.payload.city_id || '-'}
                    </div>
                  </td>
                  <td className='px-3 py-2'>
                    <div className='text-[10px]'>
                      {row.payload.contact || '-'} <br /> {row.payload.email || '-'}
                    </div>
                  </td>
  ...
  return (
    <div className='min-h-screen bg-gray-50 py-10 px-4'>
      <div className='max-w-4xl mx-auto'>
        <div className='mb-6'>
          <Link
            href='/app/cabang/list'
            className='inline-flex items-center gap-2 text-sm text-gray-600 hover:text-gray-900'
          >
            <i className='tabler-arrow-back-up'></i> Kembali ke List Cabang
          </Link>
          <h1 className='text-xl font-semibold mt-2'>Import Data Cabang</h1>
        </div>
        ...
        <div className='bg-white rounded-xl shadow-sm border p-6 space-y-6'>
          ...
          <div className='border border-blue-200 bg-blue-50 rounded-lg p-4 text-sm text-blue-700'>
            <h3 className='font-medium mb-2'>Petunjuk Import Cabang</h3>
            <ul className='list-disc list-inside space-y-1'>
              <li>Gunakan format file Excel terbaru (.xlsx)</li>
              <li>
                Sistem melakukan pengecekan berdasarkan <b>Nama Cabang</b>.
              </li>
              <li>Jika nama cabang sudah ada, data akan di-update (Upsert).</li>
              <li>
                Pastikan <b>ID Wilayah</b> (Provinsi/Kota) sesuai dengan kode BPS di database.
              </li>
              <li>
            ...
            <div className='mt-3'>
              <a className='text-blue-600 underline hover:text-blue-400 cursor-pointer' onClick={downloadTemplate}>
                <b>Download Template Excel Cabang</b>
              </a>
            </div>
          </div>
          ...
          {preview ? (
            ...
            <div>
              ...
              >
                {loading ? 'Sedang Memproses...' : mode === 'preview' ? 'Lihat Preview Data' : 'Proses Import Cabang'}
              </Button>
            </div>
          )}
        </div>
