...
import { deleteMasterSlotWaktu, fetchMasterSlotWaktuPage, postExport, resetRedux } from '../slice/index'
...
function RowAction(data: any) {
  ...
  const handleDelete = (id: string) => {
    dispatch(deleteMasterSlotWaktu(id))
    optionsOnClose()
  }
    ...
    <TableCell size='small'>
      ...
      >
        <MenuItem
          component={Link}
          sx={{ '& svg': { mr: 2 } }}
          href={`/app/master-slot-waktu/form?id=${data.row.id_master_slot_waktu}&view=true`}
          onClick={handleView}
        >
        ...
        {canEdit && [
          <MenuItem
            ...
            component={Link}
            sx={{ '& svg': { mr: 2 } }}
            href={`/app/master-slot-waktu/form?id=${data.row.id_master_slot_waktu}`}
            onClick={handleView}
          >
        ...
        <DialogDelete
          id={data.row.nama_master_slot_waktu}
          open={openConfirm}
          ...
          handleOk={() => {
            handleDelete(data.row.id_master_slot_waktu)
            setOpenConfirm(false)
          }}
          handleClose={() => {
            setOpenConfirm(false)
...
const Table = () => {
  ...
  useEffect(() => {
    if (store.delete) {
      dispatch(fetchMasterSlotWaktuPage({ page: 1, perPage: perPage, q: filter, status: selectedStatus?.value }))
      dispatch(resetRedux())
    }
  ...
  const handleChangePage = useCallback(
    (newPage: number) => {
      setPage(newPage)
      dispatch(fetchMasterSlotWaktuPage({ page: newPage, perPage: perPage, q: filter, status: selectedStatus?.value }))
    },
  ...
  const onAddForm = () => {
    router.replace('/app/master-slot-waktu/form')
  }

  const onImport = () => {
    router.replace('/app/master-slot-waktu/import')
  }
  ...
  const handleChangePerPage = (event: any) => {
    ...
    dispatch(fetchMasterSlotWaktuPage({ page: 1, perPage: newPerPage, q: filter, status: selectedStatus?.value }))
  }
    ...
    if (dataPage) {
      ...
      return {
        ...
        fields: [
          ...
          tableColumn('KODE SLOT', 'kode_slot'),
        ...
        values: values?.map((row: any) => {
          return {
            ...row,
            jam: `${row.jam_mulai?.slice(0, -3)} - ${row.jam_selesai?.slice(0, -3)}`,
            status_custom: (
              <CustomChip
                round='true'
                size='small'
  ...
  return (
    <Grid container spacing={6} sx={{ width: '100%' }}>
      ...
      <Grid size={12}>
        <Card>
          <CardHeader title='Master Slot Waktu' sx={{ paddingBottom: 0 }} />
          <Toolbar
            sx={{
              px: '1.5rem !important',
              minHeight: 'auto',
