...
import { postBatchMasterSlotWaktu, postExport, postImport, resetRedux } from '../slice'
...
export default function ImportExcelPage() {
  ...
  const handleCommit = async () => {
    ...
    try {
      ...
      const res = await dispatch(postBatchMasterSlotWaktu({ data: payloads })).unwrap()
          ...
          <table className='min-w-full text-sm'>
            <thead className='bg-gray-100 text-left'>
              <tr>
                ...
                <th className='px-3 py-2'>Kode Slot</th>
              ...
              {result.data.map(row => (
                <tr key={row.row} className={row.valid ? 'border-t bg-white' : 'border-t bg-red-50'}>
                  ...
                  <td className='px-3 py-2 font-medium text-nowrap'>
                    {row.payload.jam_mulai} - {row.payload.jam_selesai}
                  </td>
                  <td className='px-3 py-2 font-medium'>{row.payload.active}</td>
  ...
  return (
    <div className='min-h-screen bg-gray-50 py-10 px-4'>
      <div className='max-w-3xl mx-auto'>
        <div className='mb-6'>
          <Link
            href='/app/master-slot-waktu/list'
        ...
        <div className='bg-white rounded-xl shadow-sm border p-6 space-y-6'>
          <div className='border border-blue-200 bg-blue-50 rounded-lg p-4 text-sm text-blue-700'>
            ...
            <ul className='list-disc list-inside space-y-1'>
              ...
              <li>
                Jika <b>Kode Slot</b> kosong → INSERT (data baru)
              </li>
              <li>
                Jika <b>Kode Slot</b> ada → UPDATE (perbarui data)
              </li>
