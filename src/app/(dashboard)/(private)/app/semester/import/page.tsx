...
import { postBatchSemester, postExport, postImport, resetRedux } from '../slice'
...
export default function ImportExcelPage() {
  ...
  const handleCommit = async () => {
    ...
    try {
      ...
      const res = await dispatch(postBatchSemester({ data: payloads })).unwrap()
            ...
            <tbody>
              {result.data.map(row => (
                <tr key={row.row} className={row.valid ? 'border-t bg-white' : 'border-t bg-red-50'}>
                  ...
                  <td className='px-3 py-2 font-medium'>{row.payload.nama_semester}</td>
        ...
        <div className='bg-white rounded-xl shadow-sm border p-6 space-y-6'>
          <div className='border border-blue-200 bg-blue-50 rounded-lg p-4 text-sm text-blue-700'>
            ...
            <ul className='list-disc list-inside space-y-1'>
              ...
              <li>
                Jika <b>Tahun Ajaran</b> dan <b>Semester</b> kosong → INSERT (data baru)
              </li>
              <li>
                Jika <b>Tahun Ajaran</b> dan <b>Semester</b> ada → UPDATE (perbarui data)
              </li>
