...
export default function ImportExcelPage() {
  ...
  const handleConfirmCommit = async () => {
    ...
    .map(d => {
      const { nama_jenis_jam, nama_lembaga, ...payload } = d.payload;
                  ...
                  <div className="space-y-1">
                    <div>
                      ...
                      <div className="font-medium">
                        {row.payload.nama_jampel}
                      </div>
                    </div>

                    <div>
                      <span className="text-gray-500">Jenis Jam</span>
                      <div>{row.payload.nama_jenis_jam ?? '-'}</div>
                    </div>

                    <div>
                      <span className="text-gray-500">Lembaga</span>
                      <div>{row.payload.nama_lembaga ?? '-'}</div>
                    </div>

                    <div>
                      <span className="text-gray-500">Lembaga Tipe</span>
                      <div>{row.payload.lembaga_type ?? '-'}</div>
                    </div>

                    <div>
                      <span className="text-gray-500">Waktu Mulai</span>
                      <div>{row.payload.mulai ?? ''}</div>
                    </div>

                    <div>
                      <span className="text-gray-500">Waktu Selesai</span>
                      <div>{row.payload.selesai ?? ''}</div>
                    </div>
              ...
              <table className="min-w-full text-sm">
                <thead className="bg-gray-100 text-left">
                  <tr>
                    ...
                    <th className="px-3 py-2">Jenis Jam</th>
                    <th className="px-3 py-2">Lembaga</th>
                    <th className="px-3 py-2">Tipe</th>
                    <th className="px-3 py-2">Waktu Mulai</th>
                    <th className="px-3 py-2">Waktu Selesai</th>
        ...
        <div className="bg-white rounded-xl shadow-sm border p-6 space-y-6">
          <div className="border border-blue-200 bg-blue-50 rounded-lg p-4 text-sm text-blue-700">
            ...
            <ul className="list-disc list-inside space-y-1">
              ...
              <li>Jika <b>nama_jampel</b> kosong → INSERT (data baru)</li>
              <li>Jika <b>nama_jampel</b> ada → UPDATE (perbarui data)</li>
