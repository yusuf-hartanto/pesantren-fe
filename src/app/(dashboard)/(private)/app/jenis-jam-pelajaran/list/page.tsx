...
import { deleteJenisJamPelajaran, fetchJenisJamPelajaranPage, postExport, resetRedux } from '../slice/index'
...
function RowAction(data: any) {
  ...
  const handleDelete = (id: string) => {
    dispatch(deleteJenisJamPelajaran(id))
    optionsOnClose()
  }
    ...
    <TableCell size='small'>
      ...
      >
        <MenuItem
          component={Link}
          sx={{ '& svg': { mr: 2 } }}
          href={`/app/jenis-jam-pelajaran/form?id=${data.row.id_jenisjam}&view=true`}
          onClick={handleView}
        >
        ...
        {canEdit &&
          <MenuItem
            ...
            component={Link}
            sx={{ '& svg': { mr: 2 } }}
            href={`/app/jenis-jam-pelajaran/form?id=${data.row.id_jenisjam}`}
            onClick={handleView}
          >
        ...
        {canDelete && [
          ...
          <DialogDelete
            ...
            handleOk={() => {
              handleDelete(data.row.id_jenisjam)
              setOpenConfirm(false)
            }}
            handleClose={() => {
              setOpenConfirm(false)
...
const Table = () => {
  ...
  useEffect(() => {
    if (store.delete) {
      dispatch(fetchJenisJamPelajaranPage({ page: 1, perPage: perPage, q: filter }))
  ...
  const onAddForm = () => {
    router.replace('/app/jenis-jam-pelajaran/form')
  }

  const onImport = () => {
    router.replace('/app/jenis-jam-pelajaran/import')
  }
  ...
  const handleChangePage = (newPage: number) => {
    setPage(newPage)
    dispatch(fetchJenisJamPelajaranPage({ page: newPage, perPage: perPage, q: filter }))
  }
  ...
  const handleChangePerPage = (event: any) => {
    ...
    dispatch(fetchJenisJamPelajaranPage({ page: 1, perPage: newPerPage, q: filter }))
  }
    ...
    if (dataPage) {
      ...
      return {
        ...
        fields: [
          ...
          tableColumn('NAMA', 'nama_jenis_jam'),
  ...
  return (
    <Grid container spacing={6} sx={{ width: '100%' }}>
      <Grid size={12}>
        <Card>
          <CardHeader title='Jenis Jam Pelajaran' sx={{ paddingBottom: 0 }} />
          <Toolbar
            sx={{
              px: '1.5rem !important',
              minHeight: 'auto',
