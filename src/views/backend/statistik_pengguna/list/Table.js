...
// ** Store & Actions
import { getDataStatistikPengguna, getStatistikPengguna } from '../store/action'
...
const ClassList = () => {
  ...
  const store = useSelector(state => state.statistikpenggunas),
  ...
  useEffect(() => {
    if (!store.params) {
      ...
      dispatch(
        getDataStatistikPengguna(store.params)
      )
  ...
  }, [debouncedSearch, currentPage, rowsPerPage, selectedProvince, selectedRegency])
  ...
  const handleCreate = e => {
    dispatch(getStatistikPengguna(null))
  }
