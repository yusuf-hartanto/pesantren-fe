...
// ** Store & Actions
import { getTema, deleteTema } from '../store/action'
...
const handleDelete = (row) => {
  return MySwal.fire({
    ...
    if (result.value) {
      store.dispatch(deleteTema(row.id))
    }
  })
}
    ...
    },
    {
      ...
      cell: row => (
        <UncontrolledDropdown>
          ...
          <DropdownMenu right>
            {ability.can('edit', 'tema') &&
              <DropdownItem
                tag={Link}
                to={`/tema/edit/${row.id}`}
                className='w-100'
                onClick={() => store.dispatch(getTema(row))}
              >
                <Archive size={14} className='mr-50' />
                <span className='align-middle'>Edit</span>
              </DropdownItem>
            }
            {ability.can('delete', 'tema') &&
